...
test_ES_t <- function(data = NULL, percentile = NULL) {
  ...
  if(percentile <= 0.5){
    ...
    ES_X <- data[2,] - exp(data[3,])*exp(data[4,]) / (percentile*(exp(data[4,])-1)) * dt(qt(percentile, df = exp(data[4,])),df = exp(data[4,]))
    I <- ifelse(ES_X>data[1,], 1, 0)
    k <- sum(I)
    P <- pt( (ES_X-data[2,])/exp(data[3,]), df = exp(data[4,]))
    mu <- sum(P)
    sigma <- sqrt(sum(P*(1-P)))
    
    }else{
      
    ES_X <- data[2,] + exp(data[3,])*exp(data[4,]) / ((1-percentile)*(exp(data[4,])-1)) * dt(qt(percentile, df = exp(data[4,])),df = exp(data[4,]))
    I <- ifelse(ES_X<data[1,], 1, 0)
    k <- sum(I)
    P <- pt( (ES_X-data[2,])/exp(data[3,]), df = exp(data[4,]))
    mu <- sum(P)
